...
/// Derives the session keys for a public key type that matches the local key-pair.
...
/// Generates a signature of a nonce given a key-pair. This prefixes the `NONCE_PREFIX` to the
/// signature.
